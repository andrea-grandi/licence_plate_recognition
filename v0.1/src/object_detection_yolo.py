...
def getOutputsNames(net):
    # Ottieni i nomi di tutti i livelli nella rete
    layersNames = net.getLayerNames()
    # Ottieni i nomi dei livelli di output (livelli con output non connessi)
    # Gestisci entrambe le forme di `getUnconnectedOutLayers`
    try:
...
def postprocess(frame, outs):
    ...
    for idx in indices:
        # Controlla se idx è un array con un singolo elemento o un valore scalare
        idx = idx[0] if isinstance(idx, (np.ndarray, list)) else idx
        box = boxes[idx]
        left = box[0]
        top = box[1]
        width = box[2]
        height = box[3]
        drawPred(classIds[idx], confidences[idx], left, top, left + width, top + height)
